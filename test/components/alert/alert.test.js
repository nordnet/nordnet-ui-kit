...
describe('<Alert />', () => {
  ...
  it('should render Alert box when re-mounted & passed props.dismissed false', () => {
    const wrapper = shallow(
      <Alert header="header" {...defaultProps}>
        body
      </Alert>,
    ...
    const firstRender = wrapper.find('div').exists();
    expect(firstRender).to.equal(false);

    wrapper.setProps({ dismissed: false }); // trigger componentWillReceiveProps

    const secondRender = wrapper.find('div').exists();
    expect(secondRender).to.equal(true);
  });
});

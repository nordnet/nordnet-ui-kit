...
import InputDefaultStyles
  from '../../../src/components/input/input-default-styles';
import InputSelect from '../../../src/components/input/input-select';
import InputCheckboxRadio
  from '../../../src/components/input/input-checkbox-radio';
import InputCheckboxRadioStyles
  from '../../../src/components/input/input-checkbox-radio-styles';
...
describe('<Input />', () => {
  ...
  const classesRadio = shallow.context.styleManager.render(
    InputCheckboxRadioStyles,
  );
  let wrapper;
  const inputTypes = [
    {
    ...
    describe(`with type=${type.name}`, () => {
      ...
      if (type.value !== InputCheckboxRadio) {
        ...
        it('should print the label if text is entered', () => {
          expect(
            wrapper.shallow(opts).find('Label').shallow(opts).childAt(0).text(),
          ).to.equal('label');
        });
        it('should set the placeholder to the property placeholder', () => {
          if (type.value === InputSelect) {
            ...
            const wrapperWithoutValue = shallow(
              <Input
                type={type.name}
                label="label"
                placeholder="placeholder"
            ...
            expect(
              wrapper.shallow(opts).find('input.input__element').html(),
            ).to.contain('placeholder="placeholder"');
          }
        });
        it('should have class input--has-warning if warning', () => {
          wrapper.setProps({ hasWarning: true });
          expect(
            wrapper.shallow(opts).hasClass('input--has-warning'),
          ).to.equals(true);
        });
        it('should have class input--has-error if error', () => {
          wrapper.setProps({ hasError: true });
          expect(wrapper.shallow(opts).hasClass('input--has-error')).to.equals(
            true,
          );
        });
        it('should have class input--has-success if success', () => {
          ...
          expect(
            wrapper.shallow(opts).hasClass('input--has-success'),
          ).to.equals(true);
        });
      } else {
        it(`should have class ${classesRadio['input-checkbox-radio']}`, () => {
          expect(
            wrapper
              .shallow(opts)
              .find(`.${classesRadio['input-checkbox-radio']}`),
          ).to.have.length(1);
        });
        it('should have class input-checkbox-radio--has-error if error', () => {
          wrapper.setProps({ hasError: true });
          expect(
            wrapper.shallow(opts).find('.input-checkbox-radio--has-error'),
          ).to.have.length(1);
        });
        it('should have class input-checkbox-radio--has-success if success', () => {
          wrapper.setProps({ hasSuccess: true });
          expect(
            wrapper.shallow(opts).find('.input-checkbox-radio--has-success'),
          ).to.have.length(1);
        });
        it('should be unchecked by default', () => {
          expect(
            wrapper.shallow(opts).find(`input[type="${type.name}"]`).props()
              .checked,
          ).to.equal(false);
        });
        ...
        it('should become checked if unchecked and clicked', () => {
          ...
          expect(
            checkbox.find(`input[type="${type.name}"]`).props().checked,
          ).to.equal(true);
        });
        it('should render helptext if specified', () => {
          wrapper.setProps({ helpText: 'help' });
          expect(
            wrapper.shallow(opts).find('HelpText').childAt(0).text(),
          ).to.equal('help');
        });
      }
      switch (type.name) {
        case 'text':
          it('should have class input--text', () => {
            expect(wrapper.shallow(opts).hasClass('input--text')).to.equals(
              true,
            );
          });
        ...
        case 'select':
          it('should have class input--select', () => {
            expect(wrapper.shallow(opts).hasClass('input--select')).to.equals(
              true,
            );
          });
          break;
